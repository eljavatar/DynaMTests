...
my %cmd_opts;
getopts('p:v:t:s:d:o:S', \%cmd_opts) or pod2usage(1);
...
my $USE_SCAFFOLDING = defined $cmd_opts{S} ? 1 : 0;
...
foreach my $id (@ids) {
    ...
    if ($USE_SCAFFOLDING) {
        # Si el template de la clase de test se va a generar usando los scaffoldings
        # construidos a partir de Evosuite, esta validación a contnuación sería necesaria
        # ya que se debe garantizar que cada test tenga su scaffolding específico.
        ...
        #print("Valida scaffolding $file_scaffolding");
    ...
    if (!-e $path_methods) {
        print "No existe path: $path_methods\n";
        next;
    }
    ...
    foreach my $class (keys %modified_classes) {
        ...
        if ($USE_SCAFFOLDING) {
            $test_class_name = "${focal_class_name}_ESTest";
            $path_test_class = "${file_class}_ESTest.java";
        } else {
            $test_class_name = "${focal_class_name}Test";
            $path_test_class = "${file_class}Test.java";
        }
